...
class PatientProvider with ChangeNotifier {
  ...
  bool _hasMore = true;
  final int _defaultBatchSize = 7; // Taille par défaut du lot
  ...
  /// Fetch patients with optional batch size
  Future<void> fetchPatients({bool reload = false, int? batchSize}) async {
    ...
    notifyListeners();
    try {
      final int limit = batchSize ?? _defaultBatchSize;
  ...
  Future<void> fetchMorePatients({int? batchSize}) async {
    if (_isLoading || !_hasMore) return;
    return await fetchPatients(batchSize: batchSize);
  }
